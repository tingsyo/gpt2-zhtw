    ...
    for s in sentences:
        ...
        for tokens in templist:
            ...
            if tokens in history_sentences:
                ...
                continue
            if len(set(tokens))<=1:
                logging.debug("Generated senytence has only one repeated characters, skip.")
                continue
...
def generate_poem(seed_sentence, model, tokenizer, st, params):
    ...
    total_step = 0        # Setup a counter to escape infinite loops
    max_step_allowed = 100
    empty_step = 0        # Setup a counter to escape infinite loops
    while len(output)<params['total_lines']:
        ...
        # False safe for infinite loop
        total_step+=1
        if total_step>=max_step_allowed:
            logging.debug("Stopping for exceeding the max_step_allowed: "+str(total_step))
            break
        # End false safe for ifinite loop
        if len(candidates)>0:
            selected = select_next_sentence(candidates, embeddings)
        elif empty_step>3:
            seed_sentence = output[-2]
            logging.debug('Generated sentences are not qualified, use an earlier seed sentence: '+seed_sentence)
            empty_step = 0
            continue
        else:
