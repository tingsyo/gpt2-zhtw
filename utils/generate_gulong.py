...
def main():
    ...
    model = AutoModelForCausalLM.from_pretrained('../model/gpt2pt_gulong')
    # Parse seeding string
    input_ids = tokenizer.encode(args.input, return_tensors='pt')
    # Generate text
    output = []     
    for i in range(args.trials):
        generated = model.generate(input_ids, do_sample=True, max_length=args.length, repetition_penalty=1.8)
